...
describe("favorite blogs", () => {
  ...
  test("when list has only one blog equals the likes of that", () => {
    ...
    const result = listHelper.favoriteBlog(listWithOnlyOneBlog)
    assert.deepStrictEqual(result, listWithOnlyOneBlog[0])
  })
...
describe('top likes blog', () => {
    test("of empty list is zero", () => {
        const blogs = []
        const result = listHelper.mostLikeBlogs(blogs)
        assert.strictEqual(result, 0)
      })
      test("when list has only one blog equals the likes of that", () => {
        const listWithOnlyOneBlog = [
        ...
        const result = listHelper.mostLikeBlogs(listWithOnlyOneBlog)
